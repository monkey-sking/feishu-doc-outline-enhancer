      ...
      const result = await send("Runtime.evaluate", {
        expression: `
          (() => {
            ...
            const tampermonkeyScripts = scripts.filter(s =>
              s.textContent?.includes('Feishu') ||
              s.textContent?.includes('Enhancer') ||
              s.textContent?.includes('data-tm-feishu-path-injected')
            );
            ...
            return {
              ...
              tampermonkeyScriptsFound: tampermonkeyScripts.length,
              hasTampermonkey,
              scriptSrc: tampermonkeyScripts.map(s => s.src || 'inline').slice(0, 5),
              pageUrl: window.location.href,
              matchesPattern: window.location.href.includes('my.feishu.cn/docx/')
            };
          })()
        `,
        returnByValue: true,
