const http = require("node:http");

async function findFeishuDocPage() {
  return new Promise((resolve, reject) => {
    http.get("http://127.0.0.1:9222/json", (res) => {
      let data = "";
      res.on("data", (chunk) => (data += chunk));
      res.on("end", () => {
        try {
          const pages = JSON.parse(data);
          const target = pages.find(
            (p) => p.type === "page" && p.url.includes("my.feishu.cn/docx/")
          );
          if (target) {
            console.log(`Found: ${target.title}`);
            resolve(target.webSocketDebuggerUrl);
          } else {
            reject(new Error("No docx page found"));
          }
        } catch (error) {
          reject(error);
        }
      });
    }).on("error", reject);
  });
}

let ws = null;
let id = 0;
const pending = new Map();
const logs = [];

function send(method, params = {}) {
  return new Promise((resolve, reject) => {
    const msgId = ++id;
    pending.set(msgId, { resolve, reject, method });
    ws.send(JSON.stringify({ id: msgId, method, params }));
    setTimeout(() => {
      if (pending.has(msgId)) {
        pending.delete(msgId);
        reject(new Error(`${method} timeout`));
      }
    }, 5000);
  });
}

async function main() {
  const wsUrl = await findFeishuDocPage();
  ws = new WebSocket(wsUrl);

  ws.onmessage = (event) => {
    const msg = JSON.parse(event.data);

    // Capture console messages
    if (msg.method === "Runtime.consoleAPICalled") {
      const text = (msg.params.args || [])
        .map(a => a.value || a.description || "")
        .join(" ");
      logs.push(text);
      if (text.includes("Feishu") || text.includes("Enhancer")) {
        console.log("CONSOLE:", text);
      }
    }

    if (msg.id && pending.has(msg.id)) {
      const { resolve, reject } = pending.get(msg.id);
      pending.delete(msg.id);
      if (msg.error) reject(new Error(JSON.stringify(msg.error)));
      else resolve(msg.result);
    }
  };

  ws.onerror = (err) => {
    console.error("WS Error:", err.type);
    process.exit(1);
  };

  ws.onopen = async () => {
    try {
      await send("Runtime.enable");
      await send("Log.enable");

      console.log("\n=== Checking userscript execution ===");

      // Wait a bit for any logs
      await new Promise(resolve => setTimeout(resolve, 1000));

      const result = await send("Runtime.evaluate", {
        expression: `
          (() => {
            // Check if userscript ran
            const hasObserver = !!window.MutationObserver;
            const popover = document.querySelector('.workspace-suite-path-popover-new');
            const injected = document.querySelectorAll('[data-tm-feishu-path-injected="true"]');

            // Check if there's an error in enhancing
            return {
              hasPopover: !!popover,
              popoverVisible: popover ? (popover.offsetParent !== null) : false,
              injectedCount: injected.length,
              injectedItems: [...injected].map(el => el.textContent?.trim()).slice(0, 5),
              consoleLogsCount: ${logs.length}
            };
          })()
        `,
        returnByValue: true,
        awaitPromise: true,
      });

      console.log("\n=== Result ===");
      console.log(JSON.stringify(result.result.value, null, 2));

      console.log("\n=== Console Logs ===");
      logs.forEach(log => console.log(log));

      ws.close();
      process.exit(0);
    } catch (error) {
      console.error("Error:", String(error));
      process.exit(1);
    }
  };
}

main().catch(err => {
  console.error("Main error:", String(err));
  process.exit(1);
});
